$(document).ready(function() {
	...
	setTimeout(function(){
		...
		// inline aO logic - no dependency on function definition order
		$('.aoi').addClass('animate__animated animate__bounceIn');
		$('.aoi').removeClass('aoinv');
		$('.aoi').one('animationend', function() {
	...
	if($('#header-particles').length){
		particlesJS.load('header-particles', 'data/particlesjs-config.json');
	}
	var jsp, jsp_i, peu;	
	jsp = "";	
	...
	function rf() {
		$.getJSON('data/mmgrd.json', function(roX) {
	...
	function rsf() {		
		setTimeout(function(){
			$.getJSON('data/mmgd.json', function(poX) {					
					$psm_1 = poX['ps1'];
					$psm_2 = poX['ps2'];
					$psm_3 = poX['ps3'];
					$psm_4 = poX['ps4'];
	...
	function sT(tD, cms, gn, lb, sls) {
		setTimeout(function() {	
			...
			if (sls === 1) {
				$( ".mcp-p-w-p" ).removeClass('animate__animated animate__bounceIn animation-delay-100');
				$.getJSON('data/mmgl.json', function(poL) {
						$lsv1 = poL['lst'];
						$lsv2 = poL['lsp'];
	...
	function gS(step, onStep) {
		fetch('data/' + step + '.json')
			.then(function(r){ return r.json(); })
			.then(function(d){ onStep && onStep(d.html || ''); });
	}
});